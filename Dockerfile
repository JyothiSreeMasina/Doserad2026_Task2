...
# exists specifically because Task 1's repo already has its own root
# Dockerfile and two algorithms can't share one):
#   docker build --platform=linux/amd64 -f Dockerfile -t doserad2026_task2_photon_mr .
FROM --platform=linux/amd64 pytorch/pytorch:2.9.1-cuda12.6-cudnn9-runtime AS task2_photon_mr
...
COPY --chown=user:user configs/task2_photon_mr.yaml /opt/app/configs/task2_photon_mr.yaml
